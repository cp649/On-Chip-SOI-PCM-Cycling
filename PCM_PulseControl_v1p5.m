...
tam = 15*us; tcr = 0.3*s;    tprt= 500*ms; tdel = 0.4*s; % Set up amorphization and crystallization times.
Vam = 34;    Vcr = 18;     n_cycles = 40000; % Set up voltages and # of cycles you want.
start_cyc = 64416; % account for extra cycles already run on the sample
VoltPrime = [8,10,12];
% Directory to save imaged and data
ChipName = '1P_CF_THK_1'; DeviceName = '315.2';
...
%     while(getImage)
        ...
        Data = flip(Data,3);
        image = Data;
%         acTime = 80*100/mean(mean(mean(image)));
%         cam.Timing.Exposure.Set(acTime);
%         pause(acTime*ms*5);
%         cam.Acquisition.Capture(uc480.Defines.DeviceParameter.Wait);
%         % Copy image from memory
...
%      if (i<30|| (0<mod(i,25) && mod(i,25)<=2) )
    ...
    RGBAm =squeeze(mean(mean(double(image(:,:,:)))));
    RGBStdAm = std(reshape(double(image(:,:,:)),[numel(image(:,:,1)),3]));
    RGBKurtAm = kurtosis(reshape(double(image(:,:,:)),[numel(image(:,:,1)),3]));
%      end
...
%         pause(0.05);
        ...
        Data = flip(Data,3);
        image = Data;
%         if mean(mean(mean(image)))>100 || attempts>10
%             getImage = false; 
%         else
%             warning('Low brigthness for xrs, collecting image again');
%             attempts = attempts+1;
%         end
%         pause(0.05);
%     end
...
%     if (i<30|| (0<mod(i,25) && mod(i,25)<=2))
    ...
    RGBCr =squeeze(mean(mean(double(image(:,:,:)))));
    RGBStdCr = std(reshape(double(image(:,:,:)),[numel(image(:,:,1)),3]));
    RGBKurtCr = kurtosis(reshape(double(image(:,:,:)),[numel(image(:,:,1)),3]));
% end
    fprintf (fagi, '*CLS');
    fprintf (fkie, '*CLS');
    ...
    diim = (abs((imcr)-(imam)));
    R_Thr = 8; G_Thr = 8; B_Thr = 10;
    ViabPix(1) = sum(sum(imbinarize(diim(:,:,1),R_Thr)));
    ViabPix(2) = sum(sum(imbinarize(diim(:,:,2),G_Thr)));
    ViabPix(3) = sum(sum(imbinarize(diim(:,:,2),B_Thr)));

    mask = imbinarize(diim(:,:,1),R_Thr)+imbinarize(diim(:,:,2),G_Thr)+imbinarize(diim(:,:,3),B_Thr);
    mask = imbinarize(mask,0.5);
    ViabPix(4) =sum(sum(mask));

    BrtAm = repmat((~mask),1,1,3).*imam; bram = mean(mean(BrtAm));
    BrtCr =repmat((~mask),1,1,3).*imcr; brcr = mean(mean(BrtCr));
    FOMAm= mean(mean(repmat((mask),1,1,3).*imam))./bram;
    FOMCr= mean(mean(repmat((mask),1,1,3).*imcr))./brcr;
    ...
    currLog = ['cyc ',num2str(i) ' Vam ',num2str(Vam),' tam ', num2str(tam*1e6), ' us ','VoltPrime ',num2str(VoltPrime),...
        ...
        ' ViabPxl ' num2str(ViabPix),' DMAAm ', num2str(FOMAm), ' DMACr ', num2str(FOMCr) ,' Exp Time ',num2str(acTime),'\n' ];
    fprintf(fid,currLog);
    fclose(fid);
